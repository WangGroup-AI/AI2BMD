...
AI<sup>2</sup>BMD-PIMA integrates ViSNet-PIMA into AI<sup>2</sup>BMD through a "Transfer Learning-Pretraining-Finetuning" scheme. It directly transfers local fragment representations from baseline AI<sup>2</sup>BMD to ViSNet-PIMA, then pretrains non-local interactions on molecular mechanics (MM) data to assimilate empirical non-bonded interactions described by Coulomb and van der Waals, and finally finetunes with limited DFT labels, enabling accurate inter-fragment energy and force predictions at substantially reduced computational and data costs for biomolecular simulations.
