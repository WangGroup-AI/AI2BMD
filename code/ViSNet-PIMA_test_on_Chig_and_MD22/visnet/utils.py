...
class LoadFromFile(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        if values.name.endswith("yaml") or values.name.endswith("yml"):
            config_dir = os.path.dirname(os.path.abspath(values.name))
            with values as f:
                config = yaml.load(f, Loader=yaml.FullLoader)
            for key in ("dataset_root", "log_dir"):
                if isinstance(config.get(key), str) and not os.path.isabs(config[key]):
...
class LoadFromCheckpoint(argparse.Action):
    def __call__(self, parser, namespace, values, option_string=None):
        ckpt = torch.load(values, map_location="cpu", weights_only=False)
