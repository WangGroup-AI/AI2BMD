...
class ViSNetBlock(nn.Module):
    """
    Implementation of the ViSNet block, containing multiple ViS-MP layers and optional independent dipole layers.
    """
    ...
    def __init__(
        ...
        self.lmax = lmax  # Maximum spherical harmonic degree
        self.vecnorm_type = vecnorm_type  # Type of vector normalization
        self.trainable_vecnorm = trainable_vecnorm  # Whether vector normalization is trainable
        self.num_heads = num_heads  # Number of attention heads
        self.num_layers = num_layers  # Number of ViS-MP layers
        self.ind_num_layers = ind_num_layers  # Number of independent dipole layers
        self.hidden_channels = hidden_channels  # Number of hidden channels
        self.num_rbf = num_rbf  # Number of radial basis functions
        self.rbf_type = rbf_type  # Type of radial basis functions
        self.trainable_rbf = trainable_rbf  # Whether radial basis functions are trainable
        self.activation = activation  # Activation function for hidden layers
        self.attn_activation = attn_activation  # Activation function for attention
        self.max_z = max_z  # Maximum atomic number
        self.cutoff = cutoff  # Cutoff distance for neighbor search
        self.max_num_neighbors = max_num_neighbors  # Maximum number of neighbors
        
        # Define modules
        self.embedding = nn.Embedding(max_z, hidden_channels)  # Embedding layer for atomic types
        self.distance = Distance(cutoff, max_num_neighbors=max_num_neighbors, loop=True)  # Distance calculation module


        self.sphere = Sphere(l=lmax)  # Spherical harmonic transformation module
        self.distance_expansion = rbf_class_mapping[rbf_type](cutoff, num_rbf, trainable_rbf)  # Radial basis function expansion module
        self.neighbor_embedding = NeighborEmbedding(hidden_channels, num_rbf, cutoff, max_z).jittable()  # Neighbor embedding module
        self.edge_embedding = EdgeEmbedding(num_rbf, hidden_channels).jittable()  # Edge embedding module
        ...
        for i in range(num_layers):
            ...
            layer = vis_mp_class(last_layer=last_layer, **vis_mp_kwargs).jittable()
            self.vis_mp_layers.append(layer)
        
        # Independent dipole layers
        self.ind_block = ind_block
        
        if ind_block:
            self.pima_type = PIMA_charge_charge
            self.ind_dipole_layers = nn.ModuleList()
            self.interaction_matrices = InteractionMatrix(l=lmax,pima_type=PIMA_charge_charge)
            ind_dipole_kwargs = dict(
                hidden_channels=hidden_channels, 
                activation=activation, 
                vecnorm_type=vecnorm_type, 
                trainable_vecnorm=trainable_vecnorm
            ...
            for _ in range(ind_num_layers):
                layer = ind_dipole(last_layer=False, **ind_dipole_kwargs).jittable()
                self.ind_dipole_layers.append(layer)
            
            self.pole_E_layer = pole_interaction(**ind_dipole_kwargs)
    ...
    def forward(self, data: Data) -> Tuple[Tensor, Tensor]:
        ...
        :param data: Input data containing atomic types, positions, and batch information.
        :return: Output node features and vector features.
        """
        ...
        if self.ind_block:
            ...
            ind_edge_index,interaction_matrices = self.interaction_matrices(pos,batch)

            if not self.pima_type: # The first implementation method
                vec = self.pole(x).unsqueeze(1) * vec
                
                for ind in self.ind_dipole_layers:
                    dvec = ind(x, vec, ind_edge_index, interaction_matrices)
                    vec = vec + dvec
                dx = self.pole_E_layer(x, vec, ind_edge_index, interaction_matrices)
                x = x + dx
            
            else:
                vec = self.pole(x).unsqueeze(1)*vec
                q = self.charge(x).unsqueeze(1)*x.unsqueeze(1)  
                cur_vec = torch.cat((q,vec),dim=1)
                for ind in self.ind_dipole_layers:
                    dvec = ind(x,  cur_vec, ind_edge_index,interaction_matrices)
                    # dvec has shape (nAtoms, 3, hidden_channels) cur_vec has shape (nAtoms, 4, hidden_channels) add them at the given dim

                    cur_vec = torch.cat((
                        cur_vec[:, :1, :],  # Keep the first component unchanged
                        torch.add(cur_vec[:, 1:, :], dvec[:, 1:, :])  # Add dvec to the last three components
                    ), dim=1)              

                dx = self.pole_E_layer(x,  cur_vec, ind_edge_index, interaction_matrices)   
                x = x + dx     
...
class ind_dipole(MessagePassing):
    def __init__(
        self,
        hidden_channels,
        activation,
        ...
        super(ind_dipole, self).__init__(aggr="add", node_dim=0)
    ...
    def reset_parameters(self):
        ...
        for m in self.pole.modules():
            if isinstance(m, nn.Linear):  
                nn.init.xavier_uniform_(m.weight)  
                nn.init.zeros_(m.bias)
        
    def forward(self, x,  vec, edge_index, interaction_matrices):
        x = self.layernorm(x)
        vec = self.vec_layernorm(vec)
        
        p = self.pole(x)
        q = self.q_proj(x)
        # propagate_type: (p: Tensor, q: Tensor, vec: Tensor, interaction_matrices:Tensor)
        vec_out = self.propagate(
            edge_index,
            p=p,
            q=q,
            vec=vec,
    ...
    def message(self, p_i,q_j, vec_j,interaction_matrices):
        # T = self.interaction_matrix(pos_i,pos_j)
        vec_j = q_j.unsqueeze(1)*vec_j
        ind_p = torch.matmul(interaction_matrices, vec_j)
        
        dind_p = p_i.unsqueeze(1)* ind_p
        return dind_p
...
class pole_interaction(MessagePassing):
    def __init__(
        self,
        hidden_channels,
        activation,
        ...
        super(pole_interaction, self).__init__(aggr="add", node_dim=0)
    ...
    def forward(self, x,  vec, edge_index, interaction_matrices):
        ...
        vec = self.vec_layernorm(vec)
        p = self.pole(x)
        # propagate_type: (p: Tensor, vec: Tensor, interaction_matrices:Tensor)
        vec_out = self.propagate(
            edge_index,
            p=p,
            vec=vec,
    ...
    def message(self, p_i,p_j,vec_i, vec_j,interaction_matrices):
        # T = self.interaction_matrix(pos_i,pos_j)
        vec_j = p_j.unsqueeze(1)*vec_j
        ind_p = torch.matmul(interaction_matrices, vec_j)
        dEnergy = ((p_i.unsqueeze(1)*vec_i)* ind_p).sum(dim=1)
        return dEnergy
    ...
    def aggregate(
        ...
        Energy = features
        Energy = scatter(Energy, index, dim=self.node_dim, dim_size=dim_size)
        return Energy
