...
class NeighborEmbedding(nn.Module):
    ...
    def __call__(self, node_z, node_feats, senders, receivers, edge_weight, edge_feats, mask):
...
class EdgeEmbedding(nn.Module):
    ...
    def __call__(self, senders, receivers, edge_attr, x, mask):
        x_j = x[senders]
        x_i = x[receivers]

        edge_messages = self.message_fn(x_i, x_j, edge_attr)
        edge_messages = edge_messages * mask[:, None]
...
class VecLayerNorm(nn.Module):
    ...
    def __call__(self, vec):
        # 完全保留您原有的 __call__ 结构
        norm_type = VecNormType(self.norm_type)

        # (修改) 在这里加入 4，使其成为支持的维度
        if vec.shape[1] in [3, 4, 8]:
            # 这部分选择 norm_fn 的逻辑完全保持您原来的样子
            ...
            # 在原有逻辑中插入 shape=4 的处理分支
            if vec.shape[1] == 3:
                vec = norm_fn(vec)
            # +++ 新增: 只在这里添加对 shape=4 的处理 +++
            elif vec.shape[1] == 4:
                vec1, vec2 = jnp.split(vec, indices_or_sections=[1], axis=1)
                vec1 = norm_fn(vec1)
                vec2 = norm_fn(vec2)
            ...
            # 更新错误信息
            raise ValueError(f"VecLayerNorm only supports 3, 4, or 8 channels, but got {vec.shape[1]}")
