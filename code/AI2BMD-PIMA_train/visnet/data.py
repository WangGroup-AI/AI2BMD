...
class DataModule(LightningDataModule):
    ...
    def prepare_dataset(self):
        ...
        if self.hparams["task"] == "inference":
            self.idx_train = torch.empty(0, dtype=torch.long)
            self.idx_val = torch.empty(0, dtype=torch.long)
            self.idx_test = torch.arange(len(self.dataset), dtype=torch.long)
    ...
    def _prepare_rMD17_dataset(self):
        ...
        return idx_train, idx_val, idx_test
    def _prepare_Cumulene_dataset(self):

        self.dataset = Cumulene_train(root=self.hparams["dataset_root"], dataset_args=self.hparams["dataset_arg"], train_val_test="train")
        train_val_size = len(self.dataset)
        train_size = round(train_val_size * 0.95)
        val_size = round(train_val_size * 0.05)
    ...
    def _prepare_Pima_dataset(self):

        # chig_dataset = Pima(root=self.hparams["dataset_root"], dataset_args = 'chig')
        # trp_dataset = Pima(root=self.hparams["dataset_root"], dataset_args = 'trp')
        # ww_dataset = Pima(root=self.hparams["dataset_root"], dataset_args = 'ww')
        abd_dataset = Pima(root=self.hparams["dataset_root"], dataset_args = 'abd')
        #pacsin_dataset = Pima(root=self.hparams["dataset_root"], dataset_args = 'pacsin')
        self.dataset=torch.utils.data.ConcatDataset([abd_dataset]) #chig_dataset, trp_dataset, ww_dataset, , pacsin_dataset
        self.dataset.get_atomref=self.dataset.datasets[0].get_atomref
        train_val_size = len(self.dataset)
        train_size = round(train_val_size * 0.9)
        val_size = round(train_val_size * 0.09)
    ...
    def _prepare_Pima_ft_dataset(self):
        ft_dataset = Pima(root=self.hparams["dataset_root"], dataset_args = 'pima_ft')
        self.dataset=ft_dataset
        train_val_size = len(self.dataset)
        train_size = round(train_val_size * 0.9)
        val_size = round(train_val_size * 0.05)
    ...
    def _prepare_Pima_test_dataset(self):
        ft_dataset = Pima(root=self.hparams["dataset_root"], dataset_args = self.hparams["dataset_arg"])
        self.dataset=ft_dataset
        train_val_size = len(self.dataset)
        train_size = round(train_val_size * 0.0)
        val_size = round(train_val_size * 0.0)
