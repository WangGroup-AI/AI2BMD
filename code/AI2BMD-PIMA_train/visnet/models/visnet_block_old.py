...
class ViSNetBlock(nn.Module):
    ...
    def __init__(
        ...
        #self.distance_lr = Distance(100, max_num_neighbors=600, loop=True)
        ...
        '''self.vis_mp_layers_lr = nn.ModuleList()
        for _ in range(3 - 1):
            layer = vis_mp_class(last_layer=False, **vis_mp_kwargs).jittable()
            self.vis_mp_layers_lr.append(layer)
        
        self.vis_mp_layers_lr.append(vis_mp_class(last_layer=True, **vis_mp_kwargs).jittable())'''
    ...
    def reset_parameters(self):
        ...
        '''for layer in self.vis_mp_layers_lr:
            layer.reset_parameters()'''
    ...
    def forward(self, data: Data) -> Tuple[Tensor, Tensor]:
        ...
        '''edge_index, edge_weight, edge_vec = self.distance_lr(pos, batch)
        edge_attr = self.distance_expansion(edge_weight)
        mask = edge_index[0] != edge_index[1]
        edge_vec[mask] = edge_vec[mask] / torch.norm(edge_vec[mask], dim=1).unsqueeze(1)
        edge_vec = self.sphere(edge_vec)
        edge_attr = self.edge_embedding(edge_index, edge_attr, x)
        for attn in self.vis_mp_layers_lr[:-1]:
            dx, dvec, dedge_attr = attn(x, vec, edge_index, edge_weight, edge_attr, edge_vec)
            x = x + dx
            vec = vec + dvec
            edge_attr = edge_attr + dedge_attr
        dx, dvec, _ = self.vis_mp_layers_lr[-1](x, vec, edge_index, edge_weight, edge_attr, edge_vec)
        x = x + dx
        vec = vec + dvec'''
