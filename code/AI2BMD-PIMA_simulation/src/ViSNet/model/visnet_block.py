...
class ViSNetPimaBlock(nn.Module):
    def __init__(
        self,
        lmax: int=2,
        vecnorm_type="none",
        ...
        super(ViSNetPimaBlock, self).__init__()
        self.lmax = lmax
        self.vecnorm_type = vecnorm_type
        self.trainable_vecnorm = trainable_vecnorm
        self.num_heads = num_heads
        ...
        self.pole_E_layer = pole_interaction(hidden_channels,vecnorm_type).jittable()
        self.pole = nn.Sequential(
            nn.Linear(hidden_channels, hidden_channels * 2),
            nn.SiLU(),
            nn.Linear(hidden_channels*2, hidden_channels )
    ...
    def forward(self, data: dict[str, Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        x = x + dx
        vec = vec + dvec
        ind_edge_index = radius_graph(pos, r=30.0, batch=batch, loop=False, max_num_neighbors=200)
...
class ind_dipole(MessagePassing):
    ...
    def forward(self, x,  vec, edge_index: torch.Tensor, interaction_matrices:torch.Tensor):
        x = self.layernorm(x)
        vec = self.vec_layernorm(vec)
