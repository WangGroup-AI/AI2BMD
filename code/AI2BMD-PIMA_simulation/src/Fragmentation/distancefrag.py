...
class DistanceFragment(DipeptideFragment):
    ...
    def calculate_permutation_indices(
        ...
        # adjust permutation indices for FF19SB
        # if solvent_method == 'FF19SB':
        #     # original atoms are already in correct order
        #     for i, idx in enumerate([
        #         i for i, v in enumerate(permute_idx)
        #         if v < dipeptide_length - len(constrain_idx)
        #     ]):
        #         permute_idx[idx] = i

        #     if resi_state != 0:
        #         permute_idx[:2] = permute_idx[1::-1]
