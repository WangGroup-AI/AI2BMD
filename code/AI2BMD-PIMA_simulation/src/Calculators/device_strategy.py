...
class DeviceStrategy:
    ...
    def initialize(cls, dev_strategy: str, work_strategy: str, solvent_method: str, gpu_count: int, chunk_size: int):
        ...
        if solvent_method == "tinker-GPU" and gpu_count > 0:
        ...
        if not model_on_cpu:
            print("DeviceStrategy: Warning: libgomp bug detected. Lowering torch thread count to 1!")
            ...
            print(f"DeviceStrategy: Warning: libgomp bug detected, but {total_models} model(s) on CPU. Lowering torch thread count to {torch_threads}!")
            torch.set_num_threads(torch_threads)

        return { 'solvent-method': solvent_method }
