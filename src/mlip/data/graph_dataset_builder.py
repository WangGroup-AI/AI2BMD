...
class GraphDatasetBuilder:
    ...
    def prepare_datasets(self, pretrained_z_table = None) -> None:
        ...
        dataset info object.
        """
        train_systems, valid_systems, test_systems = self._reader.load(pretrained_z_table = pretrained_z_table)
        if pretrained_z_table is None:
            z_table = self._construct_z_table(train_systems)
        else:
            z_table = AtomicNumberTable(sorted(pretrained_z_table))
