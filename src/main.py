...
from AIMD.geometry_optimization import ProteinOptimizer
...
if __name__ == "__main__":
    ...
    if task_type == "optimization":
        ...
        fix_atomic_numbers(preeq_nowat_pdb, prot)
        
        # 初始化优化器
        # fmax 通常可以通过命令行参数传入，这里演示默认为 0.05
        ...
        # 执行优化
        optimizer.optimize(
            prot_name=args.prot_file,
            max_cycles=args.max_cycles, 
            max_step_size=args.max_step_size          # 限制单步位移提高稳定性
        )

    elif task_type == "simulation":
        print(f"Task: Simulation detected. Initializing Simulator...")
        ...
        simulator.simulate(
            ...
            raise ValueError(f"Unknown task: {task_type}. Please use 'simulation' or 'optimization'.")